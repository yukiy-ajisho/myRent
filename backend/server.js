...
// GET /owner/loans - Get all loans for the current owner
app.get("/owner/loans", async (req, res) => {
  try {
    const userId = req.user.id;

    console.log(`[SECURITY] User ${userId} requesting owner loans`);
    ...
    console.error("Get owner loans error:", error);
    res.status(500).json({ error: "Failed to fetch owner loans" });
  }
});
